...
\begin{itemize}
    ...
    \end{align}
    Probability mass outside $S_t$ is discarded, so this is generally not
    equal to the full-vocabulary reverse KL. It preserves multi-token
    supervision on the student's high-probability region at lower cost.
\end{itemize}
...
But what if one wants to perform the OPD with just the distillation loss with top-k KL estimation, i.e., \eqref{eq:opd.top-k-objective}? 

The cloest approximation is from the \href{https://github.com/Thinking-Space/Rethinking-OPD/tree/ac26e38d6f1572eb027597b48a9f4e01f6915ef8}{Github Repo of Rethinking-OPD}. The flow inside \icode{ppo/ray_trainer.py} is as follows:
\begin{enumerate}
...
rm_scores = -kl_val * norm_weights
    ...
    \[
        ...
        \equiv \bar{\pi}_{\theta,S_t}(i\mid s_t)  \text{ for } i\in S_t
    \]
    and the produced candidate-level reward is
    \[
        r_{t,i}
        =
        \bar{\pi}_{\theta,S_t}(i\mid s_t)
        \left[
            \log\pi_T(i\mid s_t)
            -
            \log\pi_\theta(i\mid s_t)
    ...
    \]
    This \texttt{norm\_weights} seems redundant if one consider the loss \eqref{eq:opd.token-kl} or \eqref{eq:opd.sampled-token-unbiased}. We will explain it in the following part.
