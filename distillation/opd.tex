...
\begin{enumerate}
    ...
    \newline
    \icode{gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch_output)}.
...
topk_log_probs = full_topk_log_probs[:, -response_length - 1 : -1, :]
    ...
    Therefore, for each student-visited prefix $(x,y_{<t})$,
    \[
    ...
    \[
        ...
        =
        \log \pi_\theta(i\mid x,y_{<t}) \equiv \log p_t(i)
        \qquad i\in S_t.
    \]
...
# ... omit operations to get the tensors' shapes right
    \end{codeblock}
    At this point, the last piece is on the mechanics of \href{https://github.com/Thinking-Space/Rethinking-OPD/blob/ac26e38d6f1572eb027597b48a9f4e01f6915ef8/verl/verl/workers/fsdp_workers.py#L1830}{\texttt{\_compute\_teacher\_top\_k\_log\_probs}}. It offers 5 options for selecting the top-k token sets:
    \begin{itemize}
...
# use chunked top_k to save the memory
for start in range(0, n_samples, chunk_size):
    ...
    \end{codeblock}
    The gathered values are returned as \texttt{teacher\_on\_student\_log\_probs}, so for
    \texttt{only\_stu},
    \[
        \texttt{teacher\_on\_student\_log\_probs}[i]
        =
        \log \pi_{\mathrm{Teacher}}(i\mid x,y_{<t}) \equiv \log q_t(i)
        \qquad i\in S_t,
    \]
    with the teacher logits divided by \texttt{teacher\_temperature}.
    ...
    \href{https://github.com/Thinking-Space/Rethinking-OPD/blob/ac26e38d6f1572eb027597b48a9f4e01f6915ef8/verl/verl/workers/actor/dp_actor.py#L451}{DataParallelPPOActor.compute\_distillation\_reward}. It also supports the above 5 strategies. We use the default strategy \texttt{only\_stu} to demonstrate the computation.
    \begin{codeblock}[language=Python]
...
rm_scores = -kl_val * norm_weights
    ...
    \[
        ...
        =
        \frac{\pi_\theta(i\mid x,y_{<t})}
        {\sum_{j\in S_t}\pi_\theta(j\mid x,y_{<t})}
        \equiv \bar{p}_t^{(S_t)}(i)  \text{ for } i\in S_t
    \]
    under the default \texttt{reward\_weight\_mode="student\_p"} (the alternatives
    \texttt{"teacher\_p"} and \texttt{"none"} use teacher or uniform weights, and the analysis of
    Section~\ref{sec:opd.rethinking-method} does not apply to them), and \texttt{kl\_val} is
    $\delta_t(i)$. The produced candidate-level reward is
    \begin{align}
        r_{t,i}
        =
        -\operatorname{sg}\!\left[
    ...
    $\nabla_\theta\Phi_t$, with $S_t$ replaced by that strategy's candidate set.
