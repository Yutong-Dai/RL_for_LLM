...
\begin{itemize}
    ...
    \item \textbf{GKD-OPD}: The distillation loss is the target. However, the VeRL implements the teacher-top-k, hence the forward-KL is used when the KL estimation is estimated with the top-k instead of the sampled-token.
\end{itemize}


But what if one wants to use VeRL to perform the OPD with just the distillation loss with top-k KL estimation, i.e., \eqref{eq:opd.top-k-objective}? The cloest approximation is from the \href{https://github.com/Thinking-Space/Rethinking-OPD/tree/ac26e38d6f1572eb027597b48a9f4e01f6915ef8}{Github Repo of Rethinking-OPD}.
